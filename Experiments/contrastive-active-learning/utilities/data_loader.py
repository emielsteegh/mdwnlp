...
def get_glue_dataset(
    ...
    :param task: glue task  ("sst-2", "qqp", "qnli", "ag_news", "dbpedia", "pubmed", "ornl8", "ornl26")
    :param evaluate:
    :return:
    """
    create_dir(data_dir)
    ...
    if test:
        filename = (
            "cached_{}_{}_original".format("test_contrast", str(task))
            if contrast
            else "cached_{}_{}_original".format("test", str(task))
        )
        if ood:
            filename += "_ood"
        cached_dataset = os.path.join(data_dir, filename)
    else:
        if evaluate and contrast:
            filename = "cached_{}_{}_original".format("dev_contrast", str(task))
        else:
            filename = "cached_{}_{}_original".format(
                "dev" if evaluate else "train", str(task)
            )
    ...
    if ood_name is not None:
        cached_dataset += "_{}".format(ood_name)
    ...
    if os.path.exists(cached_dataset):
        ...
        if test:
            if ood:
                examples = processor.get_test_examples_ood(data_dir)
            else:
                examples = (
                    processor.get_contrast_examples("test")
                    if contrast
                    else processor.get_test_examples(data_dir)
                )
        else:
            if evaluate and contrast:
                examples = processor.get_contrast_examples("dev")
            else:
                examples = (
...
def get_glue_tensor_dataset(
    ...
    if (
        os.path.exists(cached_features_file) and data_dir == args.data_dir
        ...
        if test:
            if ood:
                examples = (
                    ...
                    if ood_name is None
                    else (processor.get_test_examples_ood(data_dir, ood_name))
                )
            else:
                examples = (
                    processor.get_contrast_examples("test", contrast_ori)
                    if (contrast or contrast_ori)
                    else processor.get_test_examples(data_dir)
                )
        elif evaluate:
            examples = (
        ...
        if X_inds is not None:
            ...
            if hasattr(args, "annotations_per_iteration") and hasattr(
                args, "oversampling"
            ):
                if args.oversampling and len(examples) != len(X_inds):
                    new_samples_inds = list(
                        np.array(X_inds)[-args.annotations_per_iteration :]
                    )
