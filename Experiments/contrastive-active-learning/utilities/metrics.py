...
def ece_(y_prob, y_true, n_bins):
    ...
    for bin_lower, bin_upper in zip(bin_lowers, bin_uppers):
        ...
        if prop_in_bin.item() > 0:
            ...
            accuracy_in_bin = 0.0
        bin_stats[bin] = {
            "acc": float(round(accuracy_in_bin, 3)),
            "conf": float(round(avg_confidence_in_bin, 3)),
        }
...
def var_ratio_(y_prob, return_vec=False):
    ...
    return {
        "mean": float(round(var_ratio_mean, 3)),
        "var": float(round(var_ratio_var, 3)),
    }
...
def entropy_(y_prob, return_vec=False):
    ent = (
        -1.0
        * np.sum(np.multiply(y_prob, np.log(y_prob + np.finfo(float).eps)), axis=1)
        / np.log(2)
    )
...
def margin_score(y_prob, return_vec=False):
    ...
    return {
        "mean": float(round(margin_ratio_mean, 3)),
        "var": float(round(margin_ratio_var, 3)),
    }
...
def avg_spherical_score(y_prob, y_true, return_vec=False):
    ...
    prob_i = np.array([y_prob[j, y_true[j]] for j in range(len(y_prob))])
    spherical_loss = 1.0 - (
        prob_i / (np.sqrt(np.sum(np.square(y_prob))) + np.finfo(float).eps)
    )
...
if __name__ == "__main__":
    ...
    print(
        "Expected Calibration Error (Calibration, refinement, Brier Multiclass): ",
        ece_(y_pred, y_label, n_bins=15),
    )
