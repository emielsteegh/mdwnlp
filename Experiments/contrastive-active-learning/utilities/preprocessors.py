...
class MnliProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir, ood=False):
        ...
        if os.path.isfile(os.path.join(data_dir, "{}.json".format(filename_json))):
            with open(
                os.path.join(data_dir, "{}.json".format(filename_json))
            ) as json_file:
                [X_test, y_test] = json.load(json_file)
        else:

            lines = self._read_tsv(
                os.path.join(data_dir, "{}.tsv".format(filename_tsv))
            )
            X_test = []
            y_test = []
            for (i, line) in enumerate(lines):
            ...
            with open(
                os.path.join(data_dir, "{}.json".format(filename_json)), "w"
            ) as f:
...
class Trec6Processor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        if not os.path.isfile(os.path.join(data_dir, "train_42.txt")):
            self._train_dev_split(data_dir)
        return self._create_examples(
            self.read_txt(os.path.join(data_dir, "train_42.txt")), "train"
        )
    ...
    def get_dev_examples(self, data_dir):
        ...
        if not os.path.isfile(os.path.join(data_dir, "dev_42.txt")):
            self._train_dev_split(data_dir)
        return self._create_examples(
            self.read_txt(os.path.join(data_dir, "dev_42.txt")), "dev"
        )

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self.read_txt(os.path.join(data_dir, "test.txt")), "test"
        )
    ...
    def _train_dev_split(self, data_dir, seed=42):
        ...
        lines = open(
            os.path.join(data_dir, "train.txt"), "r", encoding="ISO-8859-1"
        ).readlines()
        for line in lines:
            columns = line.rstrip().split(" ")
...
class Ornl8Processor(DataProcessor):
    """Processor for the Open Rechtspraak dutch data set."""

    data_location = os.path.join(sys.path[0], "data/ORNL8")
    ...
    def get_test_examples(self, data_dir):
        if os.path.isfile(os.path.join(data_dir, "test.json")):
            ...
            dataset = load_dataset(self.data_location, split="test")
            X_test = [x["text"] for x in dataset]
            y_test = [x["label"] for x in dataset]
    ...
    def _train_dev_split(self, data_dir, seed=42, split=0.05):
        ...
        dataset = load_dataset(self.data_location, split="train")
        X = [x["text"] for x in dataset]
        Y = [x["label"] for x in dataset]
...
class Ornl26Processor(DataProcessor):
    """Processor for the Open Rechtspraak dutch data set."""

    data_location = os.path.join(sys.path[0], "data/ORNL26")
...
class CounterfactualNliProcessor(DataProcessor):
    def ind2path(self, counterfactual):
        assert counterfactual in [
            ...
            "new",
            "orig",
        ], "counterfactual {}".format(counterfactual)
    ...
    def get_train_examples(self, data_dir, counterfactual):
        ...
        return self._create_examples(
            self._read_tsv(
                os.path.join(data_dir, self.ind2path(counterfactual), "train.tsv")
            ),
            "train",
        )
    ...
    def get_dev_examples(self, data_dir, counterfactual):
        ...
        return self._create_examples(
            self._read_tsv(
                os.path.join(data_dir, self.ind2path(counterfactual), "dev.tsv")
            ),
            "dev",
        )
    ...
    def get_test_examples(self, data_dir, counterfactual):
        ...
        return self._create_examples(
            self._read_tsv(
                os.path.join(data_dir, self.ind2path(counterfactual), "test.tsv")
            ),
            "test",
        )
...
class CounterfactualSentimentProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, counterfactual):
        ...
        if counterfactual == "paired":
            return self._create_examples(
                self._read_tsv(
                    os.path.join(data_dir, "combined/paired", "train_paired.tsv")
                ),
                "train",
            )
        else:
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, counterfactual, "train.tsv")),
                "train",
            )
    ...
    def get_dev_examples(self, data_dir, counterfactual):
        ...
        if counterfactual == "paired":
            return self._create_examples(
                self._read_tsv(
                    os.path.join(data_dir, "combined/paired", "dev_paired.tsv")
                ),
                "dev",
            )
        else:
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, counterfactual, "dev.tsv")), "dev"
            )
    ...
    def get_test_examples(self, data_dir, counterfactual):
        ...
        if counterfactual == "paired":
            return self._create_examples(
                self._read_tsv(
                    os.path.join(data_dir, "combined/paired", "test_paired.tsv")
                ),
                "test",
            )
        else:
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, counterfactual, "test.tsv")),
                "test",
            )
    ...
    def get_test_examples_ood(self, data_dir, ood_dataset):
        ...
        return self._create_examples(
            self._read_tsv(
                os.path.join(
                    COUNTERFACTUAL_DATA_DIR,
                    # counterfactual,
                    "{}_balanced.tsv".format(ood_dataset),
                )
            ),
            "test",
        )
...
processors = {
    ...
    "ornl8": Ornl8Processor,
    "ornl26": Ornl26Processor,
...
output_modes = {
    ...
    "ornl8": "classification",
    "ornl26": "classification",
