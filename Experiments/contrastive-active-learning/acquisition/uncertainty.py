...
def select_alps(args, sampled, acquisition_size, dpool_inds=None, original_inds=None):
    ...
    dataset = get_glue_tensor_dataset(
        original_inds, args, args.task_name, tokenizer, train=True, evaluate=False
    )
...
def bertKM(args, sampled, acquisition_size, model, dpool_inds, original_inds=None):
    ...
    # dataset = get_glue_tensor_dataset(None, args, args.task_name, tokenizer, train=True)
    dataset = get_glue_tensor_dataset(
        original_inds, args, args.task_name, tokenizer, train=True
    )
    # X_inds=None because they are removed later (the labeled/already sampled data)
...
def margin_of_confidence(prob_dist):
    ...
    difference = [
        prob.values[x][0] - prob.values[x][1] for x in range(0, len(prob_dist))
    ]
...
def ratio_of_confidence(prob_dist):
    ...
    prob = torch.sort(prob_dist, descending=True)
    ratio_conf = np.array(
        [prob.values[x][1] / prob.values[x][0] for x in range(0, len(prob_dist))]
    )
    return ratio_conf
...
def calculate_uncertainty(
    ...
    if method not in ["random", "alps", "badge", "FTbertKM"]:
        if type(logits) is list and logits != []:
            assert init_unlabeled_data == logits[0].size(
                0
            ), "logits {}, inital unlabaled data {}".format(
    ...
    if method == "least_conf":
        ...
        uncertainty_scores, sampled_ind = BB_acquisition(
            logits, device, annotations_per_it
        )
        assert len(set(sampled_ind)) == len(sampled_ind), "unique {}, total {}".format(
            len(set(sampled_ind)), len(sampled_ind)
        )
        assert (
            len(sampled_ind) == annotations_per_it
        ), "sampled ind {}, acquisition size {}".format(
            len(sampled_ind), annotations_per_it
        )
    elif method == "random":
        pass
        ...
        raise ValueError(
            "Acquisition function {} not implemented yet check again!".format(method)
        )

    if method == "random":
        sampled_ind = np.random.choice(
            init_unlabeled_data, annotations_per_it, replace=False
        )
    elif method == "alps":
        old_labeled = labeled_inds.copy()
        old_and_new = select_alps(
            args,
            labeled_inds,
            annotations_per_it,
            candidate_inds,
        ...
        assert (
            len(old_and_new) == len(labeled_inds) + annotations_per_it
        ), "old_and_new {}, labeled {}, annotations per it {}".format(
            len(old_and_new), len(labeled_inds), annotations_per_it
        )
        sampled_ind = [x for x in old_and_new if x not in old_labeled]
        assert (
            len(sampled_ind) == annotations_per_it
        ), "sampled {}, annotations per it {}".format(
            len(sampled_ind), len(annotations_per_it)
        )
    elif method == "badge":
        old_labeled = labeled_inds.copy()
        old_and_new = badge(
        ...
        if method != "batch_bald":
            ...
            sampled_ind = np.argpartition(uncertainty_scores, -annotations_per_it)[
                -annotations_per_it:
            ]
            if args.reverse:
                sampled_ind = np.argpartition(uncertainty_scores, annotations_per_it)[
