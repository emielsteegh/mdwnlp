...
def contrastive_acquisition(
    ...
    if args.bert_score:  # BERT score representations (ablation)
        train_dataset = get_glue_tensor_dataset(
            labeled_inds, args, args.task_name, tokenizer, train=True
        )
        _train_results, train_logits = my_evaluate(
            train_dataset,
            args,
            model,
            prefix="",
        ...
        criterion = (
            nn.KLDivLoss(reduction="none") if not args.ce else nn.CrossEntropyLoss()
        )
        bert_score_matrix, bs_calc_time = calculate_bertscore(
            args, X_original, original_inds
        )
        assert bert_score_matrix.shape[0] == len(original_inds), "bs {}, ori".format(
            bert_score_matrix.shape[0], len(original_inds)
        )
        kl_scores = []
        distances = []
        pairs = []
        ...
        for unlab_i, zipped in enumerate(zip(candidate_inds, logits_dpool)):
            ...
            labeled_neighborhood_inds = np.argpartition(
                labeled_data_similarities, -args.num_nei
            )[-args.num_nei :]
            distances_ = labeled_data_similarities[labeled_neighborhood_inds]
            distances.append(distances_)
            labeled_neighbours_labels = train_dataset.tensors[3][
                labeled_neighborhood_inds
            ]
            ...
            if args.ce:
                kl = np.array(
                    [
                        criterion(
                            unlab_logit.view(-1, args.num_classes), label.view(-1)
                        )
                        for label in labeled_neighbours_labels
                    ]
                )
            else:
                uda_softmax_temp = 1
                candidate_log_prob = F.log_softmax(
                    unlab_logit / uda_softmax_temp, dim=-1
                )
                kl = np.array(
                    [
        ...
        if args.reverse:
            selected_inds = np.argpartition(kl_scores, annotations_per_iteration)[
                :annotations_per_iteration
            ]
        else:
            selected_inds = np.argpartition(kl_scores, -annotations_per_iteration)[
        ...
        if not args.knn_lab:
            ...
            for unlab_i, unlab_logit in enumerate(
                ...
                correct_prediction = (
                    True if unlab_true_label == unlab_pred_label else False
                )

                # tfidf neighbourhood
                unlab_tfidf = dpool_tfidf[unlab_i]
                distances_tfidf, neighbours_tfidf = neigh_tfidf.kneighbors(
                    X=[unlab_tfidf.numpy()], return_distance=True
                )
                labeled_neighbours_tfidf_inds = np.array(labeled_inds)[
                    neighbours_tfidf[0]
                ]  # orig inds
                labeled_neighbours_tfidf_labels = train_dataset.tensors[3][
                    neighbours_tfidf[0]
                ]
                logits_neigh_tfidf = [train_logits[n] for n in neighbours_tfidf]
                preds_neigh_tfidf = [
                    np.argmax(train_logits[n], axis=1) for n in neighbours_tfidf
                ]
                neigh_prob_tfidf = F.softmax(train_logits[neighbours_tfidf], dim=-1)

                # cls neighbourhood
                unlab_cls = dpool_cls[unlab_i]
                distances_cls, neighbours_cls = neigh_cls.kneighbors(
                    X=[unlab_cls.numpy()], return_distance=True
                )
                labeled_neighbours_cls_inds = np.array(labeled_inds)[
                    neighbours_cls[0]
                ]  # orig inds
                labeled_neighbours_cls_labels = train_dataset.tensors[3][
                    neighbours_cls[0]
                ]
                logits_neigh_cls = [train_logits[n] for n in neighbours_cls]
                preds_neigh_cls = [
                    np.argmax(train_logits[n], axis=1) for n in neighbours_cls
                ]
                neigh_prob_cls = F.softmax(train_logits[neighbours_cls], dim=-1)

                distances.append((distances_tfidf[0].mean(), distances_cls[0].mean()))
                common_neighbours_inds = [
                    x for x in neighbours_tfidf[0] if x in neighbours_cls[0]
                ]
                common_neighbours_labels = train_dataset.tensors[3][
                    common_neighbours_inds
                ]

                common_neighbours_inds_orig = list(
                    np.array(labeled_inds)[common_neighbours_inds]
                )
                common_neighbours_labels_orig = [
                    label_map[y_original[x]] for x in common_neighbours_inds_orig
                ]
                assert sorted(common_neighbours_labels_orig) == sorted(
                    common_neighbours_labels
                )

                # same predicted label with neighbourhood (percentage)
                pred_label_tfif_per = len(
                    [
                        x
                        for x in labeled_neighbours_tfidf_labels
                        if x == unlab_pred_label
                    ]
                ) / len(labeled_neighbours_tfidf_labels)
                pred_label_cls_per = len(
                    [x for x in labeled_neighbours_cls_labels if x == unlab_pred_label]
                ) / len(labeled_neighbours_cls_labels)

                # same true label with neighbourhood (percentage)
                true_label_tfif_per = len(
                    [
                        x
                        for x in labeled_neighbours_tfidf_labels
                        if x == unlab_true_label
                    ]
                ) / len(labeled_neighbours_tfidf_labels)
                true_label_cls_per = len(
                    [x for x in labeled_neighbours_cls_labels if x == unlab_true_label]
                ) / len(labeled_neighbours_cls_labels)

                nei_stats = {
                    "pred_label_tfif_per": pred_label_tfif_per,
                ...
                if args.ce:
                    kl = np.array(
                        [
                            ...
                            for label in labeled_neighbours_tfidf_labels
                        ]
                        + [
                            criterion(
                                unlab_logit.view(-1, args.num_classes), label.view(-1)
                            )
                            for label in labeled_neighbours_cls_labels
                        ]
                    )
                else:
                    uda_softmax_temp = 1
                    ...
                    kl = np.array(
                        [
                            torch.sum(criterion(candidate_log_prob, n), dim=-1).numpy()
                            for n in neigh_prob_tfidf
                        ]
                        + [
                            torch.sum(criterion(candidate_log_prob, n), dim=-1).numpy()
                            for n in neigh_prob_cls
                        ]
                    )
        ...
        if args.bert_rep and bert_representations is not None:
            ...
            if args.mean_embs and args.cls:
                dtrain_reprs = torch.cat(
                    (_train_results["bert_mean_inputs"], _train_results["bert_cls"]),
                    dim=1,
                )
                dpool_reprs = torch.cat(
                    (results_dpool["bert_mean_inputs"], results_dpool["bert_cls"]),
                    dim=1,
                )
            elif args.mean_embs:
        ...
        if not args.knn_lab:  # centroids: UNLABELED data points
            ...
            for unlab_i, candidate in enumerate(
                tqdm(
                    zip(dpool_bert_emb, logits_dpool),
                    desc="Finding neighbours for every unlabeled data point",
                )
            ):
                ...
                labeled_neighbours_inds = np.array(labeled_inds)[
                    neighbours[0]
                ]  # orig inds
                ...
                if args.ce:
                    ...
                    uda_softmax_temp = 1
                    candidate_log_prob = F.log_softmax(
                        candidate[1] / uda_softmax_temp, dim=-1
                    )
                    kl = np.array(
                        [
                            torch.sum(criterion(candidate_log_prob, n), dim=-1).numpy()
            ...
            logger.info(
                "Total Different predictions for similar inputs: {}".format(num_adv)
            )
            ...
            criterion = (
                nn.KLDivLoss(reduction="sum") if not args.ce else nn.CrossEntropyLoss()
            )
            ...
            kl_scores_per_unlab = np.array([0.0] * len(candidate_inds))
            for i, pair in enumerate(neighbours):
                ...
                if args.ce:
                    ...
                    kl = np.array(
                        [
                            criterion(
                                logits_dpool[n].view(-1, args.num_classes),
                                labeled_neighbours_label.view(-1),
                            )
                            for n in pair
                        ]
                    )
                    ...
                    kl = np.array(
                        [
                            criterion(
                                F.log_softmax(logits_dpool[n] / 1, dim=-1), labeled_prob
                            ).numpy()
                            for n in pair
                        ]
                    )
                ...
                scores = np.array(
                    [
                        np.append(kl_scores_per_unlab[pair][i], kl[i])
            ...
            # select argmax
            selected_inds = np.argpartition(
                kl_scores_per_unlab, -annotations_per_iteration
            )[-annotations_per_iteration:]
            print()

    # map from dpool inds to original inds
    sampled_ind = list(
        np.array(candidate_inds)[selected_inds]
    )  # in terms of original inds
    ...
    if args.task_name in [
        ...
        l = [
            len(sentence[0].split()) + len(sentence[1].split())
            for sentence in new_samples
        ]  # pairs of sequences
    ...
    assert (
        len(new_samples) == annotations_per_iteration
    ), "len(new_samples)={}, annotatations_per_it={}".format(
        len(new_samples), annotations_per_iteration
    )
    assert len(labeled_inds) + len(candidate_inds) + len(discarded_inds) == len(
        original_inds
    ), (
        "labeled {}, "
        "candidate {}, "
