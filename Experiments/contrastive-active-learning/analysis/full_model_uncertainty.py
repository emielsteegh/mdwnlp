...
def compute_entropy(sampled_dataset, model, args):
    ...
    dataloader = DataLoader(
        sampled_dataset, sampler=sampler, batch_size=args.per_gpu_eval_batch_size
    )
...
if __name__ == "__main__":
    ...
    ckpt_dir = os.path.join(
        CKPT_DIR,
        "{}_{}_{}_{}".format(
            args.dataset_name, args.model_type, args.acquisition, args.seed
        ),
    )
    ...
    #########################################
    path = os.path.join(EXP_DIR, "{}_{}_100%".format(args.task_name, args.model_type))
    create_dir(path)
    ...
    if len(os.listdir(args.output_dir)) != 0:
        ...
        model, tr_loss, val_loss, val_results = train_transformer(
            args, train_dataset, eval_dataset, model, tokenizer, augm_dataset
        )
    ...
    sel_data_df = pd.DataFrame(
        columns=["dataset", "acquisition", "diversity_score", "uncertainty_score"]
    )
    ...
    for dataset in datasets:
        ...
        for acq_fun in acq_funs:
            ...
            for seed in seeds:
                ...
                if os.path.exists(filepath):
                    ...
                    if sel_ids is not None:
                        ...
                        data_unsampled_inds = [
                            x for x in X_inds if x not in data_sampled_inds
                        ]
                        data_sampled = get_glue_tensor_dataset(
                            data_sampled_inds,
                            args,
                            args.task_name,
                            tokenizer,
                        ...
                        data_unsampled = get_glue_tensor_dataset(
                            data_unsampled_inds,
                            args,
                            args.task_name,
                            tokenizer,
            ...
            sel_data_df.to_csv(
                os.path.join(
                    ANALYSIS_DIR, "{}_unc_div_stats.csv".format(args.dataset_name)
                ),
                index=False,
            )
    print()
