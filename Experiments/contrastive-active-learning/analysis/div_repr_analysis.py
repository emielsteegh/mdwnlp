...
from models.deep.main_transformer import (
    get_glue_dataset,
    get_glue_tensor_dataset,
    train_transformer,
)
...
class KnnOutlierCalculator:
    def __init__(self, all_data):
        self.all_data = all_data
        self.nbrs = NearestNeighbors(n_neighbors=11, algorithm="ball_tree").fit(
            self.all_data
        )
...
if __name__ == "__main__":
    ...
    ##########################################################################
    parser.add_argument(
        "--model_type", default="bert", type=str, help="Pretrained model"
    )
    parser.add_argument(
        "--model_name_or_path",
    ...
    parser.add_argument("--tapt", default=None, type=str, help="ckpt of tapt model")
    ##########################################################################
    # Training args
    ##########################################################################
    parser.add_argument("--do_train", default=True, type=bool, help="If true do train")
    parser.add_argument(
        "--do_eval", default=True, type=bool, help="If true do evaluation"
    )
    parser.add_argument(
        "--overwrite_output_dir", default=True, type=bool, help="If true do evaluation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Dataset [mrpc, ag_news, qnli, sst-2, trec-6, ornl8, ornl26]",
    )
    parser.add_argument(
        "--task_name",
        default=None,
        type=str,
        help="Task [MRPC, AG_NEWS, QNLI, SST-2, ORNL8, ORNL26]",
    )
    parser.add_argument(
        "--max_seq_length", default=256, type=int, help="Max sequence length"
    )
    parser.add_argument(
        "--counterfactual", default=None, type=str, help="Max sequence length"
    )
    ...
    parser.add_argument(
        ...
        default="lab",
        type=str,
        help="if lab augment labeled set else unlabeled",
    )
    parser.add_argument(
        "--num_per_augm",
        default=1,
    ...
    parser.add_argument(
        "--num_augm", default=100, type=int, help="number of examples to augment"
    )
    parser.add_argument(
        "--da_all", default=False, type=bool, help="if True augment the entire dataset"
    )
    parser.add_argument(
        "--uda",
        default=False,
        type=bool,
        help="if true consistency loss, else supervised learning",
    )
    parser.add_argument(
        "--uda_confidence_thresh", default=0.5, type=float, help="confidence threshold"
    )
    parser.add_argument(
        "--uda_softmax_temp",
        default=0.4,
        type=float,
        help="temperature to sharpen predictions",
    )
    parser.add_argument(
        "--uda_coeff", default=1, type=float, help="lambda value (weight) of KL loss"
    )
    ##########################################################################
    # Server args
    ##########################################################################
    ...
    if args.tapt is not None:
        ...
        tapt_ckpt_path = os.path.join(
            CKPT_DIR, "{}_ft".format(args.dataset_name), args.tapt
        )
        # args.model_name_or_path = tapt_ckpt_path
    # Output dir
    model_type = args.model_type if args.model_type != "allenai/scibert" else "bert"
    output_dir = os.path.join(
        BERT_DIR, "{}_{}".format(args.dataset_name, args.model_type)
    )
    ...
    if args.patience is not None:
        args.output_dir += "-early{}".format(int(args.num_train_epochs))
    if args.tapt is not None:
        args.output_dir += "-{}".format(args.tapt)
    if args.da is not None:
        if args.da_all:
            args.output_dir += "-{}-{}-{}-{}".format(
                args.da, args.num_per_augm, "all", args.da_set
            )
        else:
            args.output_dir += "-{}-{}-{}-{}".format(
                args.da, args.num_per_augm, args.num_augm, args.da_set
            )
    if args.uda:
        args.output_dir += "-uda"
    ...
    if args.patience is not None:
        name = "det_{}_lr_{}_bs_{}_early_epochs_{}_{}".format(
            args.seed,
            args.learning_rate,
        ...
        name = "det_{}_lr_{}_bs_{}_epochs_{}_{}".format(
            args.seed,
            args.learning_rate,
            args.per_gpu_train_batch_size,
            args.num_train_epochs,
    ...
    # Load (raw) dataset
    X_train, y_train = get_glue_dataset(
        args, args.data_dir, args.task_name, args.model_type, evaluate=False
    )
    X_val, y_val = get_glue_dataset(
        args, args.data_dir, args.task_name, args.model_type, evaluate=True
    )
    X_test, y_test = get_glue_dataset(
        args, args.data_dir, args.task_name, args.model_type, test=True
    )
    ...
    if args.indicator is not None:
        ...
        if "sample_" in args.indicator:
            ...
            (
                X_train_orig_after_sampling_inds,
                X_train_orig_remaining_inds,
                _,
                _,
            ) = train_test_split(
                X_inds,
    ...
    if len(os.listdir(args.output_dir)) != 0:
        ...
        filepath = os.path.join(
            FIG_DIR,
            "{}_{}_{}".format(args.dataset_name, args.model_type, "random"),
            "prob_{}_{}".format(args.seed, "25_config_early20"),
        )
        ...
        init_train_dataset = get_glue_tensor_dataset(
            X_train_init_inds, args, args.task_name, tokenizer, train=True
        )
        minibatch = int(
            len(X_inds) / (args.per_gpu_train_batch_size * max(1, args.n_gpu))
        )
        ...
        eval_dataset = get_glue_tensor_dataset(
            None, args, args.task_name, tokenizer, evaluate=True
        )
        model, tr_loss, val_loss, val_results = train_transformer(
            args, init_train_dataset, eval_dataset, model, tokenizer
        )
    ...
    sel_data_df = pd.DataFrame(
        columns=["dataset", "acquisition", "diversity_score", "repr_score"]
    )
    ...
    for dataset in datasets:
        ...
        for acq_fun in acq_funs:
            ...
            for seed in seeds:
                ...
                filepath = os.path.join(
                    FIG_DIR,
                    "{}_{}_{}".format(dataset, model_name, acq_fun),
                    "prob_{}_{}".format(seed, _i),
                )
                if os.path.exists(filepath):
                    results_filename = os.path.join(
                        filepath, "results_of_iteration.json"
                    )
                    sel_ids_filename = os.path.join(
                        filepath, "selected_ids_per_iteration.json"
                    )
                    ...
                    if data_sampled_inds_1 is not None:
                        ...
                        all_data_dataset = get_glue_tensor_dataset(
                            X_inds, args, args.task_name, tokenizer, train=True
                        )
                        _, _, _results = test_transformer_model(
                            args, dataset=all_data_dataset, model=model, return_cls=True
                        )
                        ...
                        print(stats_dict)
                        seed_data_df = seed_data_df.append(
                            stats_dict, ignore_index=True
                        )
            ...
            sel_data_df.to_csv(
                os.path.join(
                    ANALYSIS_DIR, "{}_repr_div_stats.csv".format(args.dataset_name)
                ),
                index=False,
            )
    print()
